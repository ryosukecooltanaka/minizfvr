...
class Estimator:
    ...
    def __init__(self,
        ...
        self.laterality_window = laterality_window
        self.laterality_baseline_window = laterality_baseline_window
    ...
    def update_swim_estimate(self):
        ...
        if self.laterality_calc_pending:
            if last_t > (self.bout_onset_t + self.laterality_window):
                self.laterality_calc_pending = False
                in_baseline_window = (self.timestamp_buffer > (last_t - self.laterality_window - self.laterality_baseline_window)) *\
                                     (self.timestamp_buffer < (last_t - self.laterality_window))
                in_laterality_window = self.timestamp_buffer > (last_t - self.laterality_window)
