...
minizfvr tail minizftt development version 01

2025/09/22 Ryosuke Tanaka

I decided to write two completely separate apps for tail tracking and stimulus presentation, and make the tail minizftt
...
class MiniZFTT(QMainWindow):
    """
    Main tail minizftt GUI window class.
    ...
    def __init__(self):
        ...
        ## Create a minizftt object
        # The minizftt object runs in a separate child process, and continuously run the tracking algorithm on the
        # acquired camera frame that is copied into the shared memory. Then the minizftt object will send back the
        # tail angle history back to the main window through a shared memory so it can be plotted on the main GUI.
        # In addition, the minizftt object will send the tail angle with associated time stamp to whatever stimulus
